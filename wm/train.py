...
CLI:
  python -m wm.train --scale full [--v2] [--init-from ckpt] [--data-frac 0.25]
      [--tag ablation25] [--steps N] [--max-seconds S] [--out path.pt]
"""
import argparse
import json
import time
...
from wm.model import WorldModel, load_wm
...
def compute_losses(wm, batch, dev, v2, w_change):
    ...
    return total, frame_loss, rew_loss, done_loss
...
def main():
    ...
    log = {"step": [], "loss": [], "frame": [], "reward": [], "done": [],
           "val_frame": [], "val_step": []}
    t0 = time.time()
    for step in range(1, steps + 1):
        k = 1 if step <= warmup_end else K
        b = data.sample(batch, k)
        with torch.amp.autocast(dev.type, enabled=use_amp):
            total, fl, rl, dl = compute_losses(wm, b, dev, args.v2,
                                               config.WM["change_loss_weight"])
        ...
        if step % 100 == 0 or step == 1:
            ...
            print(f"  step {step}/{steps} k={k} loss={float(total):.5f} "
                  ...
                  f"done={float(dl):.5f} {time.time()-t0:.0f}s", flush=True)
        if step % 1000 == 0 or step == steps:
            wm.eval()
            with torch.no_grad(), torch.amp.autocast(dev.type, enabled=use_amp):
                vb = data.sample(min(batch, 64), K, val=True)
                _, vfl, _, _ = compute_losses(wm, vb, dev, args.v2,
                                              config.WM["change_loss_weight"])
            wm.train()
            log["val_frame"].append(float(vfl))
            log["val_step"].append(step)
            torch.save({"model": {k_: v.cpu() for k_, v in wm.state_dict().items()},
                        "config": dict(config.WM, v2=args.v2,
                                       data_frac=args.data_frac),
                        "step": step}, out)
            print(f"  [ckpt] step {step} val_frame={float(vfl):.5f} -> {out}",
                  flush=True)
    ...
    torch.save({"model": {k_: v.cpu() for k_, v in wm.state_dict().items()},
                "config": dict(config.WM, v2=args.v2, data_frac=args.data_frac),
                "step": step}, out)
